...
- 스타벅스 PPT 보고서 자동화
- 인천공항 웹리포트
- 기타 회사 내부 업무지원 프로그램 개발 및 유지보수
- 일학습병행 기업 현장교사 교육

## 1. 산자부 프로젝트 마무리

올 해가 산자부 프로젝트의 마침표를 찍는 해였다.

디지털존의 코드들을 인수인계 받고 처음 수정할 일이 생겼을 때 정말 난감했다.

jsp 파일 하나에 1000줄이 넘게 들어간 코드는 태어나서 처음 본 것 같다.

개발자들 사이에선 이런 말이 있다. 

> "사연이 없는 코드는 없다."
 
이유가 있겠지 하고 이해해보려고 해도 아니 이건 쫌 너무한거 아닌가 싶었다...

크롤링 방식도 좀 복잡스럽게 해놔서 파악하는데 정말 힘들었다.

그래도 이젠 건드릴 일이 없으니 정말 다행이라고 생각한다.
...
처음 최호철 이사님께로부터 받은 c# 코드를 보고 당황스러웠다.

프로그램을 실행하면 처음엔 PPT가 잘 편집이 되다가 중간에 멈춰버렸다.
...
기업현장교사 기본과정을 마치고 심화과정도 있다고 하여 이왕 시작한김에 심화과정까지 마무리 했다.
...
## 5. 그리고 현재 진행 및 예정인 회사 내부 업무지원 프로젝트들...

### 인천공항 웹 리포트

작년 프로젝트에 이어 올 해는 내가 맏아 진행을 했다.

통계 수치가 잘 안 맞거나 중간에 문항 유형이 추가됐었는데 나는 모르고있어서 수정 이슈가 몇 번 있었다.

난이도는 어렵진 않았는데 아마 이 때 크롤링도 같이 하고 있어서 꼼꼼함이 부족했던 것 같다.

한동안 연락이 없는걸 보니 잘 쓰고계신 것 같아 다행이다.
...
메타서베이 다운로더도 간간히 업데이트를 진행하고 있다. 

데이터 업데이트 부분에 신규로 요청사항을 받아 수정해야 하는데 시간이 없어서 계속 밀리고 있다 ㅠㅠ

## 2024년 Todo list

1. **피앰아이 기술스택 개발 공부(Vue3, Nest.js, MongoDB, Docker…) ⌛**

    Main Frontend 기술이 Vue 에서 Next.js로 바뀌었기 때문에 새로운 문물에 적응을 하는 중이다.
...
5. **마라톤 풀코스 기록 단축 ❌**
   ...
   하프 이후에 아마 공덕 쯤 지나고 탈진해서 전철을 탈까 말까 수백번 고민했다.
 
   그래도 잠실대교는 건너야지 하는 마음으로 이 악물고 버텼다.
  
   잠실대교를 건너고 10Km 남은 시점에서 아깝기도 했고 포기하고 싶지 않아서 결국 5시간 걸려서 완주했다. 
